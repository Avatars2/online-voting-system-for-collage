// Professional Messages Utility for Frontend
...
export function formatDeletionSummary(data, itemType) {
  let message = `${itemType} has been successfully deleted.`;
  
  if (data && Object.keys(data).length > 0) {
    message += "\n\nDeleted items:\n";
    
    if (data.deletedStudents > 0) {
      message += `• ${data.deletedStudents} student(s)\n`;
    }
    if (data.deletedTeachers > 0) {
      message += `• ${data.deletedTeachers} teacher(s)\n`;
    }
    if (data.deletedHod > 0) {
      message += `• ${data.deletedHod} HOD(s)\n`;
    }
    if (data.deletedClasses > 0) {
      message += `• ${data.deletedClasses} class(es)\n`;
    }
...
// Helper function to get user-friendly error message from API response
export function getApiErrorMessage(error) {
  ...
  return getMessage('error', 'SERVER_ERROR');
}
